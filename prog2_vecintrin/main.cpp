...
void printVectorBool(__cs149_mask v)
{
  for (int i = 0; i < VECTOR_WIDTH; i++)
  {
    printf("%s,", v.value[i] ? "true" : "false");
  }
  printf("\n");
}
...
void clampedExpVector(float *values, int *exponents, float *output, int N)
{
  ...
  //
  __cs149_vec_float x;
  __cs149_vec_float max_float = _cs149_vset_float(9.999999f);
  __cs149_vec_int exp;
  __cs149_vec_float result = _cs149_vset_float(1);
  __cs149_vec_int zero = _cs149_vset_int(0);
  __cs149_vec_int one = _cs149_vset_int(1);
  __cs149_mask maskExpGtZero, maskAvaliable, maskSet9f;

  for (int i = 0; i < N; i += VECTOR_WIDTH)
  {
    maskAvaliable = _cs149_init_ones();
    maskExpGtZero = _cs149_mask_not(maskAvaliable);
    maskSet9f = _cs149_mask_not(maskAvaliable);
    _cs149_vset_float(result, 1, maskAvaliable);
    // handle for last element not  fill full vector
    if ((N - i) < VECTOR_WIDTH)
    {
      for (int t = VECTOR_WIDTH - 1; t >= (N - i); t--)
    ...
    _cs149_vload_float(x, values + i, maskAvaliable);
    _cs149_vload_int(exp, exponents + i, maskAvaliable);

    _cs149_vgt_int(maskExpGtZero, exp, zero, maskAvaliable);

    _cs149_vset_float(result, 1, maskAvaliable);

    while (_cs149_cntbits(maskExpGtZero))
    {

      _cs149_vmult_float(result, result, x, maskExpGtZero);

      _cs149_vsub_int(exp, exp, one, maskExpGtZero);
      _cs149_vgt_int(maskExpGtZero, exp, zero, maskAvaliable);
    }
    _cs149_vgt_float(maskSet9f, result, max_float, maskAvaliable);
    _cs149_vset_float(result, 9.999999f, maskSet9f);
    _cs149_vstore_float(output + i, result, maskAvaliable);
  }
}
